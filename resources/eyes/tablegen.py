...
Usage:
    ...
    python genall.py <output_dir> [display_type]
...
DISPLAY_CONFIGS = {
    ...
    "trgb":    {"width": 480, "height": 480, "mapRadius": 240},  # Square display, 480x480 resolution
}
...
def generate_polar_maps(screenWidth, screenHeight, mapRadius, eyeRadius, irisRadius, slitRadius=0):
...
def generate_displacement_map(screenWidth, screenHeight, mapRadius, eyeRadius):
    ...
    for y in range(halfH):
        ...
        for x in range(halfW):
            ...
            if d2 <= eyeRadiusSq:
                d = math.sqrt(d2)
                h = math.sqrt(max(eyeRadiusSq - d2, 0))  # Height on sphere surface
                if d > 0:
                    # Angle from center
                    angle = math.atan2(dx, dy)  # This gives angle in the XY plane
...
def generate_eyelid_lookup(image, screenWidth, screenHeight):
    ...
    for x in range(screenWidth):
        ...
        for y in range(screenHeight):
            if not found_start:
                # First white pixel from top is start of eyelid
                if pixels[x, y] > 127:
                    lookup[x * 2] = y
                    found_start = True
            elif not found_end:
                # First black pixel after white is end of eyelid
                if pixels[x, y] < 127:
                    lookup[x * 2 + 1] = y
                    found_end = True
                    break
        ...
        if not found_end:
            # Eyelid goes to bottom of screen
...
def generate_eye_header(outputDir, config, screenWidth, screenHeight, mapRadius):
    """Generate complete eye header file."""
    
    eyeName = config.get("name", "eye")
    eyeRadius = config.get("radius", screenHeight // 2)
    ...
    pupilMin = pupil.get("min", 42)
    pupilMax = pupil.get("max", 200)
    
    # Iris config
    iris = config.get("iris", {})
    irisRadius = iris.get("radius", eyeRadius // 2)
    ...
    # Calculate display type name for file naming
    # Check both dimensions to properly distinguish 480x480 T-RGB from 320x240
    ...
    with open(headerPath, "w") as f:
        ...
        f.write(f"// Eye configuration for {screenWidth}x{screenHeight} display\n")
        f.write(f"// Map radius: {mapRadius}\n\n")
        
        # Generate polar maps
        polarAngle, polarDist = generate_polar_maps(
            screenWidth, screenHeight, mapRadius, eyeRadius, irisRadius, slitRadius
        )
        
        # Output polar angle map
        angleName = f"polarAngle_{mapRadius}"
        f.write(f"const uint8_t {angleName}[{mapRadius * mapRadius}] PROGMEM = {{\n")
        output_hex_array(f, polarAngle)
        f.write("};\n\n")
        
        # Output polar distance map
        distName = f"polarDist_{mapRadius}_{eyeRadius}_{irisRadius}_{slitRadius}"
        f.write(f"const uint8_t {distName}[{mapRadius * mapRadius}] PROGMEM = {{\n")
        output_hex_array(f, polarDist)
        f.write("};\n\n")
        ...
        dispName = f"disp_{mapRadius}_{eyeRadius}_{halfW}_{halfH}"
        f.write(f"const uint8_t {dispName}[{halfW * halfH}] PROGMEM = {{\n")
        output_hex_array(f, dispMap)
        f.write("};\n\n")
        
        # Eyelid lookup tables
        ...
        if upperFilename:
            upperImg = Image.open(upperFilename)
            ...
            f.write(f"// No upper eyelid defined\n")
            f.write(f"const uint8_t {upperName}[{screenWidth} * 2] PROGMEM = {{0}};\n\n")
        
        if lowerFilename:
            lowerImg = Image.open(lowerFilename)
            ...
            f.write(f"// No lower eyelid defined\n")
        ...
        if irisFilename:
            ...
            irisImg = Image.open(irisFilename).convert("RGB")
            irisW, irisH = irisImg.size
            f.write(f"// Iris texture: {irisW}x{irisH}\n")
        ...
        if scleraFilename:
            ...
            scleraImg = Image.open(scleraFilename).convert("RGB")
            scleraW, scleraH = scleraImg.size
            f.write(f"// Sclera texture: {scleraW}x{scleraH}\n")
        ...
        f.write(f"      \"{eyeName}\", {eyeRadius}, 0x{backColor:04X}, ")
        f.write(f"{\"true\" if tracking else \"false\"}, {squint}, {dispName},\n")
        
        # Pupil
        f.write(f"      {{ 0x{pupilColor:04X}, {slitRadius}, {pupilMin}, {pupilMax} }},\n")
        
        # Iris
        if irisDataName:
            f.write(f"      {{ {irisRadius}, {{ {irisDataName}, {irisDataName}Width, {irisDataName}Height }}, ")
        else:
            f.write(f"      {{ {irisRadius}, {{ nullptr, 0, 0 }}, ")
...
def generate_all_eyes(configDir, outputDir, displayType="default"):
    ...
    print(f"Generating eyes for {displayType}: {screenWidth}x{screenHeight}, mapRadius={mapRadius}")
