...
// Get render buffer for DMA
    ...
    // Full framebuffer rendering (for displays that need batch transfer)
    bool beginFramebuffers();
    void renderFullFrame(float eyeX, float eyeY, float pupilFactor,
                        uint16_t irisAngle, uint16_t scleraAngle);
    uint16_t* getFramebuffer() { return m_framebuffer; }
...
private:
    ...
    // Full framebuffer for batch rendering
